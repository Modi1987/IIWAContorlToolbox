%% ICT iiwa control toolbox

% Copyright Mohammad SAFEEA, 2nd-Oct-2019
...
classdef ICT < handle
    ...
    properties (SetAccess = protected) % public)
        ...
        RobotType='';
        FlangeType=0;
        inertialDataOfTool=[0,0,0,0];
        % oreder of data mass [COMx,COMy,COMz]
    end
    
    methods
        
        % Constructor method:
        function this = ICT(robot_ip,robotType,h_flange, varargin)
            if nargin==5
                temp=varargin{1};
			...
			return;
                ...
                temp=varargin{2};
                % assigin Teftool the input value
                if(sum(sum(size(temp)==[1,4]))==2)
                    this.inertialDataOfTool=temp;
                else
                    error('Size of passed inertial data of tool is not 1x4');
			return;
                end
            elseif nargin==4
                temp=varargin{1};
                ...
                if(sum(sum(size(temp)==[4,4]))==2)
                    this.Teftool=temp;
                    this.inertialDataOfTool=[0,0,0,0];
                    disp('Inertial data of the tool was assigned to [0,0,0,0]')
                else
                    error('Size of passed transofmration matrix is not 4x4');
			return;
                end
		...
		return;
            ...
            %% Print some data
            disp('An instance of ICT object was created')
            disp('The transofmration matrix of the Tool Center Point is:')
            disp(this.Teftool)
            disp('The inertial data of the attached Tool are:')
            fprintf('Mass: %f (kg)\n',this.inertialDataOfTool(1));
            fprintf('COMx: %f (meter)\n',this.inertialDataOfTool(2));
            fprintf('COMy: %f (meter)\n',this.inertialDataOfTool(3));
            fprintf('COMz: %f (meter)\n',this.inertialDataOfTool(4));
        end
        ...
        function [flag]=net_establishConnection( this )
            ...
            if ~exist('t','var') || isempty(t) || strcmp(t.Status,'closed')
              error('Connection could not be establised, script aborted');
                ...
                this.t_Kuka=t;
                T=this.Teftool;
                inertias_=[this.inertialDataOfTool(1),this.inertialDataOfTool(2:end)*1000]; % convert COMx, COMy and COMz to (mm)
                ...
                [y]=attachToolToFlange(t,transForm,inertias_);
                pause(0.4);
                if(y==true)
                    flag=1;
                else
